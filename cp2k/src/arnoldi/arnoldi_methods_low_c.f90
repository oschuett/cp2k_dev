...
! **************************************************************************************************
  SUBROUTINE arnoldi_init_c (matrix, vectors, arnoldi_data)
   ...
   ! create a local data copy to store the vectors and make Gram Schmidt a bit simpler
    ...
    CALL dbcsr_scale(vectors%input_vec%prv, REAL(1.0, real_4)/rnorm)
    ...
    DO i=1, SIZE(matrix)
       CALL dbcsr_matrix_vector_multiply(matrix(i)%matrix, &
                                         ...
                                         alpha=CMPLX(1.0, 0.0, real_4), &
                                         beta=CMPLX(0.0, 0.0, real_4), &
...
! **************************************************************************************************
  SUBROUTINE transfer_dbcsr_to_local_array_c(vec, array, n, is_local)
    ...
    data_vec => dbcsr_get_data_p(vec%prv, select_data_type=CMPLX(0.0, 0.0, real_4))
